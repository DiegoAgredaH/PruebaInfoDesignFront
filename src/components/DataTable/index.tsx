...
export function DataTable({ columns, data, component }: Props) {
  ...
  return (
    <div className="w-full animate-in slide-in-from-left-20 duration-300">
      <div className="flex items-center py-4">
        <Input
          placeholder="Filtrar por Linea..."
          value={(table.getColumn("Linea")?.getFilterValue() as string) ?? ""}
          onChange={(event) =>
            table.getColumn("Linea")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="ml-auto">
              Columnas <Icons.chevronDownIcon className="ml-2 h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
